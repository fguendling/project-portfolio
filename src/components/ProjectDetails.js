...
let PROJECTS = [
  ...
  {
    id: 7,
    content: "We are supporting the client by introducing new infrastructure. It is cloud-based, meaning that resources are deployed and hosted on third-party providers. The client uses this infrastructure for application development purposes and to offer different services to its customers. The core of this new infrastructure is based on Kubernetes. We are taking care of the deployments and the configuration of the Kubernetes clusters. and are responsible for their availability. Furthermore, various other products are deployed to the environments, such such as logging- and monitoring solutions (e.g.,: Grafana), or software for protecting the existing services (e.g.,: CloudFlare). The infrastructure gets declared with Hashicorp's Terraform (Infrastructure as Code) and is versioned in GitLab, from where it is also deployed automatically. We are making use of Helm charts to install add-ons into the Clusters.",
  },
];
